...
const TradingAdvisor: React.FC = () => {
  ...
  const runAdvisor = async () => {
    ...
    try {
      const res = await apiClient.get(`/v1/advisor/${ticker.toUpperCase()}?positionValue=${positionValue}&lookbackDays=${lookbackDays}`);
      setData(res.data);
    } catch {
        ...
        <>
          ...
          <div style={card}>
            ...
            <div style={{ marginTop: 8, fontSize: 12, color: '#475569' }}>
              Signals are based on Fast/Slow moving-average crossovers.
            </div>
          </div>
        </>
      )}
